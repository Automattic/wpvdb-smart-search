<?php
/**
 * WordPress site search surface tests.
 *
...
use WPVDB_Smart_Search\Surfaces\Site_Search;

/**
 * Tests site search replacement behavior.
 *
 * @covers \WPVDB_Smart_Search\Surfaces\Site_Search
 */
final class SiteSearchTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$GLOBALS['wpvdb_smart_search_test']['transients']      = [
			Site_Search::EMBEDDINGS_TRANSIENT => '1',
		];
	...
	/**
	 * Test eligible main search queries are marked for site search handling.
	 *
	 * @covers \WPVDB_Smart_Search\Surfaces\Site_Search::mark_query
	 */
	public function test_mark_query_flags_eligible_search_query(): void {
		$query = new WP_Query( [ 's' => 'markets' ] );

		Site_Search::mark_query( $query );

		self::assertTrue( (bool) $query->get( Site_Search::QUERY_FLAG ), 'Eligible search queries should be flagged.' );
	}

	/**
	 * Test non default fields requests bypass site search handling.
	 *
	 * @covers \WPVDB_Smart_Search\Surfaces\Site_Search::mark_query
	 */
	public function test_mark_query_bypasses_non_default_fields(): void {
		...
		self::assertSame( '', $query->get( Site_Search::QUERY_FLAG ), 'Site search should not alter fields=ids queries.' );
	}

	/**
	 * Test ranked pools are cached before per request visibility filtering.
	 *
	 * @covers \WPVDB_Smart_Search\Surfaces\Site_Search::pre_query
	 */
	public function test_pre_query_caches_pool_and_filters_readable_posts(): void {
		...
		$query->set( Site_Search::QUERY_FLAG, true );

		$page_one = Site_Search::pre_query( null, $query );

		self::assertIsArray( $page_one, 'Site search should return hydrated posts.' );
		...
		$page_two_query->set( Site_Search::QUERY_FLAG, true );

		$page_two = Site_Search::pre_query( null, $page_two_query );

		self::assertIsArray( $page_two, 'Site search should return hydrated posts on cached pages.' );
	...
	public function test_pre_query_preserves_existing_posts_from_earlier_filter(): void {
		...
		$posts = Site_Search::pre_query( $existing, $query );

		self::assertSame( $existing, $posts, 'Site search should not replace results supplied by an earlier posts_pre_query filter.' );
	...
	public function test_pre_query_revalidates_late_query_mutations(): void {
		...
		$posts = Site_Search::pre_query( null, $query );
	...
	public function test_pre_query_returns_zero_on_search_error_when_fallback_disabled(): void {
		$GLOBALS['wpvdb_smart_search_test']['options'][ Settings::OPTION_NAME ]['site_search_fallback'] = false;
	...
	public function test_pre_query_reuses_cached_pool_across_status_filters(): void {
		...
		$publish_query->set( Site_Search::QUERY_FLAG, true );
		Site_Search::pre_query( null, $publish_query );
		...
		$private_query->set( Site_Search::QUERY_FLAG, true );
		$posts = Site_Search::pre_query( null, $private_query );
